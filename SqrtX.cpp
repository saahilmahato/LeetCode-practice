 ...
 *    - Here, it’s applied to a *mathematical search space* — the range [1, x/2].
 *    - Elegant proof that binary search is a *pattern of thought*, not just a data structure trick.
 *
 * 2. **Problem Reformulation**
 *    - We’re essentially finding the *largest integer `mid` such that* `mid² ≤ x`.
 *    - This transforms a continuous math problem into a discrete search problem.
 *    - Beautiful example of *quantization* — turning real math into integer logic.
 *
 ...
 *    - Recognizing monotonic behavior is the foundation of all binary-search-based numeric algorithms.
 *
 ...
 *    - Even if we overshoot and the loop ends, `ans` holds the correct truncated sqrt.
 ...
 *    - Binary search is not just “find index”; it’s “find boundary where condition flips.”
 ...
 *    - Understanding these bounds is a subtle math trick baked into algorithm design.
 *
 * 10. **Binary Search as a Numerical Tool**
 *     - This is a stepping stone to **Newton’s method**, **binary root finding**, and **floating-point refinement**.
 *     - Many advanced algorithms reuse this skeleton for higher-precision calculations.
 *
 * 11. **Philosophical Reflection**
 *     - Binary search here mirrors a meditative practice — halving uncertainty step by step until clarity (or `ans`) emerges.
 *     - The answer isn’t guessed; it’s *narrowed into existence*.
 *
 * 🚀 TL;DR:
 * `mySqrt()` is a brilliant showcase of **binary search abstraction**, **overflow safety**, 
 * and **mathematical reasoning turned into code**.  
 * It’s where you realize binary search isn’t about data — it’s about decisions.
 */
