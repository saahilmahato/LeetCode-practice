 ...
 *    - Encourages analyzing time complexity and realizing the need for better approaches.
 *
 * 2. **Hash Map Lookup Trick**
 *    - The optimal O(n) solution uses a hash map to store values as you iterate.
 *      → Key: number value
 *      → Value: index
 ...
 *    - Core transferable pattern to countless problems (e.g. "two-sum variants", "counting pairs", "subarray sum equals k").
 *
 * 5. **Index vs. Value Thinking**
 *    - Separating *values* and *indices* helps avoid logic confusion.
 *    - Trains attention to detail on problem statement constraints (return indices, not numbers).
 *
 ...
 *        → 3Sum, 4Sum, Two Sum II, Subarray problems, and dynamic programming transitions.
 *
 * 7. **Tradeoff Awareness**
 *    - Understand when extra memory (hash map) is worth time savings.
 *    - Builds foundational understanding of *time vs. space complexity tradeoffs*.
 *
