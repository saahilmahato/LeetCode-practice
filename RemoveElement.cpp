 ...
 *    - For each element:
 *        → If it’s *not equal* to the target `val`, keep it by writing it to
 * `nums[j]`. → If it *is* equal, skip it — effectively “removing” it.
 *    - `j` always tracks the next position to write a *valid* element.
 *
 * 2. **In-Place Filtering**
 *    - No extra space — modifies the array in-place.
 *    - Teaches memory efficiency and reinforces *data overwrite instead of
 * deletion*.
 ...
 *    - Builds intuition for *state tracking via counters* instead of data
 * structures.
 *    - Teaches how data transformation can happen implicitly via index
 * arithmetic.
 *
 * 4. **Iterative Filtering Mindset**
 *    - Treats the array as a data stream:
 *        → Keep valid elements, skip invalid ones.
 *    - Same logic pattern applies to filtering logs, inputs, and datasets in
 * real-world code.
 *
 * 5. **In-Place Overwrite vs. Delete Tradeoff**
 *    - Actual elements aren’t deleted; they’re overwritten by valid ones.
 *    - Teaches that *removal* in low-level languages usually means “ignore from
 * now on”, not literal memory deletion.
 *
 * 6. **Time and Space Complexity Awareness**
 *    - O(n) time → single linear pass through the array.
 *    - O(1) space → constant extra memory.
 *    - Reinforces that “in-place” doesn’t mean “instantaneous” — it’s still
 * linear, but efficient.
 *
 ...
 *    - The **core skill:** use two pointers to rewrite only the “valid”
 * portion.
 *
 ...
 * - Reinforces that small index-based algorithms often power high-level
 * systems.
 *
 * TL;DR: Remove Element teaches **in-place filtering**, **two-pointer data
 * streaming**, and **index-controlled state mutation** — a foundational skill
 * for writing efficient, elegant array code.
 */
