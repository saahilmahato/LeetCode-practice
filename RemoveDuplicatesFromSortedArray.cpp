 ...
 *    - Core pattern: one pointer (`j`) tracks the position of the last unique element, 
 *      the other (`i`) scans through the array.
 *    - `j` = slow pointer → where to write the next unique value.  
 *      `i` = fast pointer → scans ahead to find new unique elements.
 *    - This design efficiently removes duplicates *in-place* without extra memory.
 *
 ...
 *    - Key lesson: updating the array in-place requires careful index management.
 *
 * 3. **Sorted Input Assumption**
 *    - Relies on sorted order → duplicates always appear consecutively.
 *    - Trains awareness of how **input properties** (sortedness) simplify algorithm design.
 ...
 *    - Elegant one-line logic that merges both identification and overwrite steps.
 ...
 *    - Demonstrates the difference between *last index position* and *array length*.
 *    - Reinforces the habit of thinking in inclusive vs. exclusive index ranges.
 *
 ...
 *    - Two-pointer techniques generalize to arrays, linked lists, and string manipulation.
 *
 * 8. **Algorithmic Elegance**
 *    - Compact yet powerful — no nested loops, no auxiliary space.
 *    - Emphasizes how *index arithmetic + conditionals* can replace complex structures.
 *    - This is the kind of low-level control that defines real algorithmic craftsmanship.
 *
 ...
 * 🚀 Bonus Insight
 * ----------------
 * - Builds *muscle memory* for two-pointer logic — a key skill in array/string problems.
 * - Teaches precision: off-by-one errors, pointer syncing, and when to increment each index.
 * - Reveals the beauty of in-place mutation: minimalism in both memory and motion.
 *
 * TL;DR: Remove Duplicates from Sorted Array teaches **two-pointer synchronization**, 
