 ...
 * where each `digits[i]` is a single digit. The digits are ordered from most significant to least significant.
 ...
 *    - This problem directly simulates how addition works by hand — column by column, right to left.
 ...
 *    - If a digit < 9, we just add one and return immediately — no need to process further.
 ...
 *    - `digits.insert(digits.begin(), 1)` expands the array by one digit only if necessary.
 ...
 *
 * 🚀 TL;DR:
 * `plusOne()` is a crisp exercise in **carry propagation**, **reverse traversal**, 
 * and **early exit optimization** — a miniature model of how addition works at the algorithmic level.
 */
